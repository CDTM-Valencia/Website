        ...
        <Tooltip delayDuration={100}>
          <TooltipTrigger asChild>
            ...
            >
              <Trophy
                className={`w-6 h-6 ${
                  placement === 1 ? 'text-yellow-900' : 'text-gray-700'
                }`}
              />
              <span className="absolute text-sm font-bold">{placement}</span>
            </div>
          </TooltipTrigger>
          <TooltipContent>
            <p>
              {placement === 1
                ? `1st Place of ${caseName} Case`
                : `2nd Place of ${caseName} Case`}
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
        ...
        <div className="container mx-auto py-12 px-4 min-h-screen">
          <header className="text-center mb-16">
            <h1 className="text-4xl font-bold mb-4">
              Projects from CDTM Valencia Hacks 2025
            </h1>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Explore the innovative solutions created by our talented teams.
              Each project represents a unique approach to solving real-world
              challenges.
            </p>
          </header>
          ...
          <p className="text-gray-600 mb-8">
            Each case is sponsored by a company and focuses on a particular
            ...
            {sortedCases.map((caseKey) => (
              <section key={caseKey}>
                <div className="flex items-center gap-3 mb-4">
                  <a
                    href={cases[caseKey as keyof typeof cases].sponsorUrl}
                    target="_blank"
                ...
                <p className="text-gray-600 mb-4">
                  {cases[caseKey as keyof typeof cases].description}
                </p>
                <div className="grid grid-cols-1 gap-8 overflow-visible relative">
                  {projectsByCase[caseKey].map((project) => (
                    ...
                    >
                      {getPlacementBadge(
                        project.placement,
                        cases[caseKey as keyof typeof cases].name
                      )}
          ...
          <div className="mb-8">
            <p className="text-gray-600">
              In addition to the cases, teams had the opportunity to compete for
              special prizes sponsored by our partners.
            </p>
          </div>
          <div className="space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-visible relative">
              {Object.entries(challenges).map(([challengeKey, challenge]) => (
                <Card
                  key={challengeKey}
                  className="relative bg-springPaleBlue/30 backdrop-blur-sm border border-springBlue/10 z-0"
                >
                  <CardHeader>
                  ...
                  <CardContent>
                    {projectsByChallenge[challenge.sponsoredBy]?.length > 0 ? (
                      <div className="space-y-2 rounded-lg">
                        {projectsByChallenge[challenge.sponsoredBy].map(
                          (project) => (
                            <div
                              key={project.id}
          ...
          <div className="mt-24 mb-16">
            ...
            <p className="text-springText/80 text-center mb-12 max-w-2xl mx-auto">
              ...
              Their support and commitment to innovation help us create an
